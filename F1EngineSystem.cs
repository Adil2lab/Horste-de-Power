...
public class GearboxSettings
{
    ...
    public float gearShiftTime = 0.05f; // 50 ms shift time
...
public class F1EngineSystem : MonoBehaviour
{
    ...
    void UpdateEngineRPM()
    {
        ...
        wheelSpeed = vehicleRigidbody.linearVelocity.magnitude;
        wheelRPM = (wheelSpeed * 60f) / (2f * Mathf.PI * 0.33f); // 0.33m wheel radius
    ...
    void ApplyEngineForces()
    {
        ...
        wheelRPM = (currentRPM / gearbox.gearRatios[gearbox.currentGear - 1]) / gearbox.finalDriveRatio;
