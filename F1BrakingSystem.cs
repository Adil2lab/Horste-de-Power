...
public class F1BrakingSystem : MonoBehaviour
{
    ...
    public BrakeSysSettings frontBrakes;
    public BrakeSysSettings rearBrakes;
    ...
    void InitializeBrakeData()
    {
        foreach (var brake in allBrakes)
        {
            BrakeSysSettings settings = IsFrontBrake(brake) ? frontBrakes : rearBrakes;
    ...
    void FixedUpdate()
    {
        ...
        vehicleSpeed = vehicleRigidbody.linearVelocity.magnitude;
    ...
    void UpdateWheelSlip(float deltaTime)
    {
        foreach (var brake in allBrakes)
        {
            ...
            float vehicleForwardSpeed = Vector3.Dot(vehicleRigidbody.linearVelocity, wheelForward);
